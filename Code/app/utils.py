...
# Lazily initialize the sentence transformer model to avoid heavy startup cost
model = None

def get_sentence_transformer():
    global model
    if not HAS_ST:
        logging.warning("sentence_transformers not available; similarity features disabled.")
        return None
    if model is None:
...
def extract_text_from_resume(file_bytes: bytes, filename: str) -> str | None:
    """
    Extracts text content from resume bytes depending on file extension.

    Supports: PDF (via pdfplumber), DOCX (via python-docx when available).
    For other formats (DOC, PNG, JPG), returns None to indicate unsupported.
    ...
    Returns:
        Extracted text, or None if unsupported/extraction failed.
    """
    ...
    try:
        ...
        # Not supported: doc, png, jpg, jpeg (would require extra deps like textract/pytesseract)
        return None
    except Exception as e:
        logging.error(f"Failed to extract text from resume {filename}: {e}")
        return None
...
def compute_similarity(cv_text, job_description):
    ...
    if sentence_model is None or not HAS_ST or util is None:
        logging.warning("Similarity model unavailable; returning default similarity 0.0")
        return 0.0
