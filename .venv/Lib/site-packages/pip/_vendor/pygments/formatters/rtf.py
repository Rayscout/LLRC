"""
LLRC Header Start
文件功能: 通用 Python 脚本/模块：.venv/Lib/site-packages/pip/_vendor/pygments/formatters/rtf.py
创建时间: 2025-08-27 11:21
创建人: 侯东杨
更新记录:
- 2025-08-27 11:51 by 李雨梦
LLRC Header End
"""
"""
...
class RtfFormatter(Formatter):
    ...
    def _escape(self, text):
        """函数 _escape：处理 text 相关逻辑。"""
        return text.replace('\\', '\\\\') \
                   .replace('{', '\\{') \
                   .replace('}', '\\}')

    def _escape_text(self, text):
        """函数 _escape_text：处理 text 相关逻辑。"""
    ...
    def hex_to_rtf_color(hex_color):
        """函数 hex_to_rtf_color：处理 hex_color 相关逻辑。"""
        if hex_color[0] == "#":
