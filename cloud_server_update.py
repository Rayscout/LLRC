...
def check_server_environment():
    ...
    # 检查Python版本
    python_version = run_command("python3 --version", "检查Python版本", True)
    if python_version:
        print(f"   🐍 Python版本: {python_version}")
    
    # 检查系统信息
    system_info = run_command("uname -a", "检查系统信息", True)
    if system_info:
        print(f"   💻 系统: {system_info}")
    
    # 检查磁盘空间
    disk_space = run_command("df -h /var/www", "检查磁盘空间", True)
    if disk_space:
        print(f"   💾 磁盘空间: {disk_space}")
    
    # 检查内存使用
    memory_info = run_command("free -h", "检查内存使用", True)
    if memory_info:
...
def backup_current_version():
    ...
    backup_dir = f"/var/www/llrc_backup_{int(time.time())}"
...
def update_code_from_git():
    ...
    run_command("git reset --hard origin/RayScout", "切换到RayScout分支")
