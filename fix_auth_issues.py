...
SECRET_KEY=your-secret-key-here-{secret}
FLASK_ENV=production
MONGODB_URI=mongodb://localhost:27017/llrc
DATABASE_URL=sqlite:///instance/site.db
""".format(secret=os.urandom(16).hex())
...
def fix_application_config():
    """修复应用配置"""
    print("\n🔧 修复应用配置...")
    
    # 检查配置文件
    config_files = [
        "/var/www/llrc/app/__init__.py",
        "/var/www/llrc/app/config.py"
    ]
    ...
    for config_file in config_files:
        if os.path.exists(config_file):
            ...
            if stat.st_uid != 1000:  # llrcuser的UID
                run_command(f"sudo chown llrcuser:llrcuser {config_file}", f"修复文件所有者: {config_file}")
