...
def get_model():
    """Get the sentence transformer model, initializing it if necessary."""
    global model
    if model is None:
        try:
...
def compute_similarity(cv_text, job_description):
    ...
    if model_instance is None:
        # Return a dummy similarity score for testing
        return 0.5

    embeddings_cv = model_instance.encode(cv_text, convert_to_tensor=True)
    embeddings_job_desc = model_instance.encode(job_description, convert_to_tensor=True)
